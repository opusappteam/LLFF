   ...
   **/
   ...
   function showForm($ID, $options=array()) {
	 ...
	 //akk retrive from customfields 
	  ...
	  $query_check_exists = "Select * from tbl_customfields where Ticket_ID= ".$_GET["id"] ; 
      ...
      if ($result = $DB->query($query_check_exists)) {
          ...
          while ($data = $DB->fetch_assoc($result))
		  {
			    ...
			    if($data['Custom_field_id']==1)
	             ...
	             if($data['Custom_field_id']==2)
	                $Reasonpopup_chose2='Selected';
	...
	//akk retrive from customfields 
      ...
      //akk
	     echo "<tr class='tab_bg_1' > ";
        echo "<th>".$tt->getBeginHiddenFieldText('Custom Fields');
      printf(__('%1$s%2$s'), __('Custom Fields'), $tt->getMandatoryMark('urgency'));
      echo $tt->getEndHiddenFieldText('Custom Fields')."</th>";

         echo "<td>";
		 echo "<select id='CustomFields' name ='CustomFields' >
    
               <option    $defaultpopup    value='0'>None</option>
    			<option  $Reasonpopup_chose1   value='1'>Choice 1</option>
			   <option   $Reasonpopup_chose2  value='2'>Choice 2</option>
			   <option  $Reasonpopup_chose3   value='3'>Choice 3</option>
